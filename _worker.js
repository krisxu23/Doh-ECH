/**
 * Total-ECH Pages 完整版（带 Cache API 缓存 + NS 自动归属识别）
 * 路由： /         -> 查询页面
 *        /api/query -> JSON API
 *        /ech      -> DoH 注入 ECH
 *        /doh      -> DoH 纯转发
 * 缓存策略：上游 DNS JSON 结果缓存到 Edge（A/AAAA 300s，HTTPS 600s，NS 3600s）
 */
...
const NS_CACHE_TTL = 86400 * 1000;       // NS 归属缓存 24 小时
...
async function handleDnsQuery(rawBuffer, config, ctx) {
    try {
        ...
        // 非静态域名：优先 NS 归属探测，后备 CIDR 探测
        let ownerData = await getOwnerFromCache(qName);
        let probedIps = null;
        if (!ownerData) {
            const probeResult = await activeProbeOwner(qName, ctx); // 内部先 NS 后 CIDR
            if (probeResult) {
        ...
        if (qType === 1 || qType === 28) {
            ...
            // CF 域名（通过 NS/CIDR 判断）才使用 cf 多域名替换
            if (ownerData === 'CF') {
                if (qType === 1 && config.metaIp4) { // 允许 metaIp4 强制覆盖 CF 的 A 记录
...
async function resolveDNS(domain, type, config) {
    ...
    const owner = await detectOwner(domain); // 内部优先 NS
    if (!ech && type === 'HTTPS') {
...
// ========== 核心：带 Cache API 的上游查询（支持 NS type=2） ==========
async function queryUpstreamDNS(name, type) {
    ...
    if (result && typeof caches !== 'undefined' && caches.default) {
        try {
            // A/AAAA 300s, HTTPS 600s, NS 3600s
            const maxAge = (type === 65) ? 600 : (type === 2 ? 3600 : 300);
...
async function detectOwner(domain) {
    // 先查内存缓存
    const cached = getOwnerFromCache(domain);
    if (cached) return cached;

    // 1. 尝试 NS 记录查询
    try {
        const nsData = await queryUpstreamDNS(domain, 2); // type NS
        if (nsData && nsData.Answer) {
            for (const rec of nsData.Answer) {
                if (rec.type === 2) {
                    const nsName = rec.data.toLowerCase();
                    // Cloudflare NS 特征
                    if (nsName.includes('.ns.cloudflare.com')) {
                        setOwnerCache(domain, 'CF', NS_CACHE_TTL);
                        return 'CF';
                    }
                    // Meta/Facebook NS 特征
                    if (nsName.includes('.ns.facebook.com') || nsName.includes('.ns.meta.com')) {
                        setOwnerCache(domain, 'META', NS_CACHE_TTL);
                        return 'META';
                    }
                }
            }
            ...
            for (const rec of data.Answer) {
                if (rec.type === 1) {
                    ...
                    if (isIpInCidrs(ip, getCompiledMeta())) {
                        setOwnerCache(domain, 'META', CACHE_TTL);
                        return 'META';
                    }
                    if (isIpInCidrs(ip, getCompiledCF())) {
                        setOwnerCache(domain, 'CF', CACHE_TTL);
                        return 'CF';
                    }
                }
            }
    ...
    // 未识别
    setOwnerCache(domain, null, 60000); // 缓存 unknown 1 分钟，避免频繁查询
    return null;
}

async function activeProbeOwner(domain, ctx) {
    try {
        const owner = await detectOwner(domain); // 内部已含 NS 优先
        if (owner) {
            // 如果能探测到归属，顺便获取 A 记录 IP（Meta 可能需要用）
            const data = await queryUpstreamDNS(domain, 1);
            if (data && data.Answer) {
...
// ========== 内存缓存辅助 ==========
function getOwnerFromCache(name) {
...
function setOwnerCache(name, owner, ttl = CACHE_TTL) {
    cacheMap.set(name, { val: owner, expire: Date.now() + ttl });
}
...
<head>
    ...
    <title>ECH DNS 查询（NS 自动识别）</title>
    <style>
        :root {
            --bg: #0f172a;
            --card: #1e293b;
...
<body>
    <div class="container">
        <h1>🔒 ECH DNS 查询（NS 自动识别）</h1>
        <p class="subtitle">自动通过 NS 记录识别 Cloudflare/Meta 站点并注入 ECH，不再依赖静态列表</p>
        <div>
            <label for="domain">域名</label>
            <input type="text" id="domain" placeholder="任何 Cloudflare 网站" value="example.com">
        </div>
        <div>
